...
impl TestRepo {
    ...
    fn write(&self, rel: &str, content: &str) {
        std::fs::write(self.dir.path().join(rel), content).expect("write file");
    }
...
fn default_branch(branches: &[gitpulse_lib::engine::BranchInfo]) -> &str {
    branches
        .iter()
...
fn checkout_default_remote_config_wins_over_guesses() {
    ...
    repo.commit_all("c1");
    run_git(repo.dir.path(), &["branch", "dev"]);
    run_git(repo.dir.path(), &["branch", "prod"]);
