...
impl WorkbenchState {
    ...
    fn request(&self, method: &str, input: &str) -> Result<Value, WorkbenchError> {
        ...
        if matches!(
            ...
            // On-device generation does not come through here. The Apple engine
            // runs the model on the calling thread via
            // `cmd_apple_intelligence_draft` and publishes the result with
            // `enhancements.complete`, so an Apple proposal never reaches
            // `enhancements.generate` — that transition hands a proposal to an
            // external worker and takes a lease against it, and there is no
            // external worker for the on-device path. Everything that arrives
            // here is genuinely destined for the sidecar.
