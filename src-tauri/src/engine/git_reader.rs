...
impl GitReader {
    ...
    ///
    /// A numstat walk that fails twice never fabricates numbers: its rows
    /// keep authoritative paths/codes with zero counts while
    /// [`StatusPayload::stats_degraded`] says the counts are missing, and a
    /// record that decodes but carries unparseable counts rides its row as an
    /// explicit warning instead of reading as fact.
    fn status_from_porcelain(
        repo: &Path,
        stdout: &str,
        fetch_numstat: impl Fn(&Path, bool) -> (NumstatParse, bool),
    ...
    fn blame_porcelain(
        ...
        Ok(BlameResult {
            lines: parse_blame_porcelain_with_base(&stdout, base_line),
            truncated: false,
        })
    }
    ...
    pub fn get_commit_details(repo_path: &str, commit_id: &str) -> Result<CommitDetails, String> {
        ...
        // silently-empty changes; totals describe the WHOLE commit even when
        // the shipped list is truncated for the UI.
        let all_files = Self::commit_files_uncapped(&repo, commit_id)?;
    ...
    pub fn list_tags(repo_path: &str) -> Result<Vec<TagInfo>, String> {
        // Newest-first (creatordate desc), matching the summary listing and
        // the recency-oriented UI contract; the frontend groups and sorts
        // independently.
        Ok(Self::list_tags_summary(repo_path)?.tags)
    }
...
fn parse_numstat_z(stdout: &str) -> Vec<NumstatZEntry> {
    ...
    while let Some(record) = records.next() {
        ...
        let entry = |path: String| NumstatZEntry {
            pathspec: format!(":(literal){}", c_unquote_path(&path)),
            path: c_unquote_path(&path),
...
/// Same walk numbered from `base_line`: a `-L` window blames a slice of the
/// file but each wire row must still carry its absolute 1-based line number.
/// See [`blame_header_oid`] for how the commit oid is recognised across hash
/// algorithms.
fn parse_blame_porcelain_with_base(stdout: &str, base_line: usize) -> Vec<BlameLine> {
...
fn parse_numstat_with_issues(stdout: &str) -> NumstatParse {
    ...
    while let Some(head) = tokens.next() {
        ...
        // Defensive decode: -z output is never C-quoted, so this is a no-op
        // for well-formed input and a repair if any invocation ever drops
        // the flag (names with quotes/control chars stay quoted regardless).
        let path_field = c_unquote_path(fields.next().unwrap_or(""));
        ...
        if path_field.is_empty() {
            if let Some(first_path) = tokens.next().filter(|s| !s.is_empty()) {
                ...
                let first_path = c_unquote_path(first_path);
                if is_rename {
                    let second_path = tokens.next().unwrap();
                    out.churn.insert(first_path, (add, del));
                    out.churn.insert(c_unquote_path(second_path), (add, del));
                } else {
                    out.churn.insert(first_path, (add, del));
                }
            }
        } else {
            out.churn.insert(path_field, (add, del));
        }
    }
    out
}
...
fn parse_numstat_files(stdout: &str) -> Vec<CommitFileChange> {
    ...
    while let Some(head) = tokens.next() {
        ...
        // Defensive decode: no-op for -z output, repair for quoted names.
        let path_field = c_unquote_path(fields.next().unwrap_or(""));
        if path_field.is_empty() {
            if let Some(first_path) = tokens.next().filter(|s| !s.is_empty()) {
                let is_rename = tokens
                ...
                if is_rename {
                    ...
                    files.push(CommitFileChange {
                        path: c_unquote_path(second_path),
                    ...
                    files.push(CommitFileChange {
                        path: c_unquote_path(first_path),
...
/// Fetches one status numstat decode with a single retry; reports `true` (the
/// degraded flag) only when BOTH attempts fail. A healthy repo gets exactly
/// the numbers git reported; a record that decodes but carries unparseable
/// counts rides its row as a warning via [`NumstatParse::issues`].
fn numstat_with_retry(repo: &Path, cached: bool) -> (NumstatParse, bool) {
    ...
    match git_text(repo, &args) {
        Ok(stdout) => (parse_numstat_with_issues(&stdout), false),
...
mod tests {
    ...
    fn numstat_ok(cached: bool) -> impl Fn(&Path, bool) -> (NumstatParse, bool) {
    ...
    #[test]
    fn c_unquote_path_decodes_c_quoted_paths_defensively() {
        // Belt-and-braces decode for any path that arrives C-quoted despite
        // quotepath=off (names with quotes/control characters are always
        // quoted by git regardless of config).
        assert_eq!(c_unquote_path("\"\\346\\227\\245.txt\""), "日.txt");

        // Quotes/control characters stay quoted even with quotepath off.
        assert_eq!(c_unquote_path("\"we\\trd.txt\""), "we\trd.txt");

        // Passthrough of unquoted names is untouched.
        assert_eq!(c_unquote_path("plain.txt"), "plain.txt");
    }
    ...
    fn tag_listing_caps_at_newest_two_hundred_tags() {
        ...
        // Plain listing: same capped membership, newest-first (creatordate
        // desc) per the recency contract the integration suite pins.
        let legacy = GitReader::list_tags(&repo_str(dir.path())).expect("legacy tags");
        assert_eq!(legacy.len(), REFS_TAG_CAP);
        assert_eq!(legacy[0].name, format!("tag-{:03}", total - 1));
    ...
    fn double_numstat_failure_marks_status_degraded_but_keeps_paths() {
        ...
        let always_fails = |_repo: &Path, _cached: bool| {
            (
                NumstatParse {
                    churn: HashMap::new(),
                    issues: Vec::new(),
    ...
    fn test_blame_porcelain_accepts_sha256_oid_length() {
        ...
        let lines = parse_blame_porcelain_with_base(
            &format!(
                "{}{}",
        ...
        let lines = parse_blame_porcelain_with_base(&blame_block(&oid40, "gamma"), 1);
    ...
    fn test_blame_porcelain_pins_oid_length_after_first_header() {
        ...
        let lines = parse_blame_porcelain_with_base(&stream, 1);
        assert_eq!(lines.len(), 2);
        assert_eq!(
