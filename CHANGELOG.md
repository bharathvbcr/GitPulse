...
- **The terminal dock belongs to a repository tab, not the workspace.** Opening a
  ...
  spawns a PTY — and spends one of the 16 global session slots — in repositories
...
- **Live-session marks on repository tabs.** A repository tab carries a terminal
  ...
  where you are not looking is visible — and so "what is using the 16 slots" is
