...
describe("annotated but unregistered commands", () => {
  it("finds every #[tauri::command] in the crate, including async and attribute-separated ones", () => {
    ...
    // 231 since cmd_agent_defaults and cmd_agent_defaults_save; `npm run check:ipc` prints all three
    // numbers, so a bump made without re-running it will not agree.
    expect(found.size).toBe(231);
    ...
    expect(found.has("cmd_codeintel_search")).toBe(true);
    expect(found.has("cmd_codeintel_impact_at_rung")).toBe(true);
