...
function fixture() {
  ...
  for (const dir of ["scripts", "src-tauri/vendored", "upstream/crates/dc-glob/src"]) mkdirSync(path.join(root, dir), { recursive: true });
  ...
  writeFileSync(path.join(upstream, "crates/Cargo.toml"), '[workspace]\n[workspace.package]\nversion = "1.0.0"\n');
  writeFileSync(path.join(upstream, "crates/dc-glob/Cargo.toml"), '[package]\nname = "dc-glob"\nversion.workspace = true\n');
  writeFileSync(path.join(upstream, "crates/dc-glob/src/lib.rs"), "pub const VALUE: u8 = 7;\n");
  ...
  const run = (...args: string[]) => spawnSync(process.execPath, [path.join(root, "scripts/vendor-crates.mjs"), ...args, "--json"], {
    ...
    env: { ...process.env, GITPULSE_ALLOW_DRIFT: "0", GITPULSE_MANVI_ROOT: upstream, GITPULSE_DEVCOUNCIL_ROOT: "/missing", GITPULSE_MARKDEV_ROOT: "/missing" },
  });
...
it("detects upstream deletions and inherited manifest changes", () => {
  const f = fixture();
  try {
    rmSync(path.join(f.upstream, "crates/dc-glob/src/lib.rs"));
    writeFileSync(path.join(f.upstream, "crates/Cargo.toml"), '[workspace]\n[workspace.package]\nversion = "2.0.0"\n');
...
it.each(["--crate=dc-glob", "full"])("preserves every old byte when %s preparation fails", (mode) => {
  ...
  try {
    ...
    writeFileSync(path.join(f.upstream, "crates/dc-glob/src/lib.rs"), "changed bytes\n");
    if (mode !== "full") writeFileSync(path.join(f.upstream, "crates/dc-glob/Cargo.toml"), '[package]\nname = "dc-glob"\nmissing.workspace = true\n');
...
it.runIf(process.platform !== "win32")("rejects source symlinks instead of copying content outside the module", () => {
  ...
  try {
    ...
    symlinkSync(path.join(f.root, "outside.rs"), path.join(f.upstream, "crates/dc-glob/src/escape.rs"));
...
it("keeps repeated source, manifest, and deletion updates coherent", () => {
  ...
  try {
    for (let generation = 0; generation < 12; generation++) {
      const source = path.join(f.upstream, "crates/dc-glob");
...
it("updates one crate without reading or rewriting unrelated upstreams", () => {
  ...
  try {
    for (const dir of ["scripts", "src-tauri/vendored/untouched", "upstream/crates/dc-glob/src"]) mkdirSync(path.join(root, dir), { recursive: true });
    for (const file of ["vendor-crates.mjs", "usage.mjs", "columns.mjs"]) cpSync(new URL(file, import.meta.url), path.join(root, "scripts", file));
    writeFileSync(path.join(root, "upstream/crates/Cargo.toml"), "[workspace]\n[workspace.package]\nversion = \"1.0.0\"\n");
    writeFileSync(path.join(root, "upstream/crates/dc-glob/Cargo.toml"), "[package]\nname = \"dc-glob\"\nversion.workspace = true\n");
    writeFileSync(path.join(root, "upstream/crates/dc-glob/src/lib.rs"), "pub const VALUE: u8 = 7;\n");
    ...
    const run = (crate: string) => spawnSync(process.execPath, [path.join(root, "scripts/vendor-crates.mjs"), `--crate=${crate}`, "--json"], {
      encoding: "utf8", env: { ...process.env, GITPULSE_MANVI_ROOT: path.join(root, "upstream"), GITPULSE_DEVCOUNCIL_ROOT: "/missing", GITPULSE_MARKDEV_ROOT: "/missing" },
    });
