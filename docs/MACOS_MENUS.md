...
Enable **Settings → Layout → Menu bar status icon**. The menu bar shows one monochrome pulse icon. Optional counts beside the glyph are **off by default** (Settings → Show counts beside the icon).

**Click routing (verified on macOS 27):** the status item does **not** keep an `NSMenu` attached at rest. Left click toggles the status popover; right click temporarily attaches the native escape menu, shows it, then detaches it again. Phase 0 instrumentation dumps button/overlay frames in debug builds and logs every `TrayIconEvent`. On this machine the tray-icon overlay covers the button; the earlier defect was the attached menu eating left clicks. No separate `NSStatusBarButton` action was required.

The glyph is a 4× supersampled brand pulse template, with an attention-dot variant when the workspace tone is warning, a parked operation is present, or live updates are degraded.

**Glyph weight (fixed after 1.1.0):** the template is a 36px canvas that `tray-icon` hands to AppKit at 18pt, so one canvas pixel is half a point. The pulse was stroked 2px wide, which measured **1pt** in a capture of the real menu bar — thinner than the count digit printed beside it (1.5pt) and than every neighbouring icon (1.5–2.5pt), with over half the mark left as anti-aliasing fringe, and it lost most of its coverage again on a 1× display where the bitmap is halved. `STROKE_PX` is now 3, measured at **2pt** on screen. `status_glyph_carries_menu_bar_stroke_weight` guards this with the same statistic the capture measures: the longest run of at-least-half-covered pixels per column, quarter-point over the columns that carry ink, which reads 2px at the old weight and 3px at the new one.

**Clearing the count (fixed after 1.1.0):** `tray-icon`'s macOS `set_title` only calls `setTitle:` inside an `if let Some`, so passing `None` clears nothing. Switching the counts preference off, closing the last repository or opening a bare one therefore stopped updating a count that stayed on the button for the rest of the session. `tray::apply` sends an empty title instead, which is what `None` already meant on GTK and is ignored on Windows either way. `native_status_material` reproduces this against a real `NSStatusItem`: before the fix it reads back `12 · ⚠3` where it must read back nothing.

**Menu-bar-only mode:** with the status icon on, **Hide Dock icon while closed** (default on) switches the app to `NSApplicationActivationPolicyAccessory` after the main window hides, and restores `Regular` when Open GitPulse / Settings / Quit reveal it.

**Launch at login** registers a per-user LaunchAgent via `tauri-plugin-autostart` with `--background` so a login start stays in the menu bar when the status icon is enabled. The Settings toggle reads `isEnabled()` from the OS; GitPulse does not store a second preference for it.

**Security impact (autostart):** enabling Launch at login writes a per-user LaunchAgent that starts GitPulse at login with `--background`. It does not add network access, broaden filesystem rights, or change IPC capabilities beyond the `autostart:*` permissions granted to the main window.

Click the icon to open a compact popover. Its monochrome header shows the repository switcher and branch (per-repo changed/conflict/busy badges in the switcher). Three equal cards show **Changed**, **Staged** and **Conflicts** in large type; nonzero counts open the relevant view. A short upstream row shows ahead/behind counts and a real **fetched … ago** / **never fetched** caption from `FETCH_HEAD` mtime. One blue primary action changes with context: **Review changes**, **Resolve conflicts**, **View history** or **Try again**. Active Git work and parked operations remain visible in the headline. Empty and unavailable states avoid misleading zero counts. Names truncate in the compact view and remain available in tooltips/details. Escape collapses the switcher or details first, then dismisses the panel; `R` refreshes, `1`–`3` open nonzero cards and `4` opens listed stashes in Work.
...
- Checkmarks reflect the active view, section, repository, Terminal and explicit appearance preference. The Terminal checkmark and its Show/Hide label follow the ACTIVE repository tab, because the dock belongs to one repository rather than the workspace: switching to a repository whose terminal is closed says Show Terminal. Labels show Show/Hide Terminal and work in progress. Native checkmarks are reconciled after clicks and recent-menu rebuilds.
...
The interactive fixture at `harness/status.html` renders the production popover with sample data and a labeled simulated backdrop. Its Material control switches between Glass and Opaque. Run `npm run test:webkit -- --harness status` for **53 real WebKit checks** covering the three-card layout and native window bounds, Details and its preserved actions, repository switching, keyboard navigation, loading/error/empty states, conflicts, long names, dark appearance, text/semantic-color contrast over extreme backdrops, live accessibility fallback changes, native padding/filter rules, the fixture's glass tuning read back against the native material in both appearances, active work and parked operations. These fixture checks validate the rendered component and action dispatch, not the native tray anchor or IPC transport.
...
The current implementation covers the six agreed enhancement groups: live availability, live selection/labels, repository switching/history, Git actions, utility actions and the status icon, plus the later menu-bar polish (click routing, Dock accessory mode, fetch age, optional tray title, and Launch at login). Further independent features include named workspace files, repository initialization, focused-editor Find/Replace/Save menus, customizable shortcuts, navigation history, advanced remote/tag/stash pickers, multiple independent windows and provider-backed CI summaries. Those require their own interaction and lifecycle contracts and are not implied by a compact status menu.
