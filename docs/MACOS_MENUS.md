...
Click the icon to open a compact popover. Its header shows the repository switcher and branch. Three cards show **Changed**, **Staged** and **Conflicts**; nonzero counts open the relevant view. A short upstream row shows ahead/behind counts from the **last fetch**. One primary action changes with context: **Review changes**, **Resolve conflicts**, **View history** or **Try again**. Empty and unavailable states avoid misleading zero counts. Names truncate in the compact view and remain available in tooltips/details.

**Details** expands within the panel for path, branch, changed/staged/conflict counts, listed stashes, upstream, watcher state, parked operations and running Git actions. A bounded scroll area keeps the footer reachable. **Refresh** and repository switching stay in the popover. **Open GitPulse**, **Settings** and **Quit** sit in the footer; right-clicking the icon also provides these three native escape actions. Light/dark appearance and reduced motion follow the main app's preferences and system settings.
...
`src-tauri/src/desktop/actions.rs` owns native IDs and the native projection of the 15 sections. `menu.rs` constructs and updates the application menu; `tray.rs` owns the icon and native right-click menu; `popover.rs` owns the lazy status window, positioning, dismissal and restricted action bridge. `state.rs` validates presentation. `desktop/mod.rs` owns event routing, main-thread updates and window/exit behavior.
...
The existing Tauri dependency enables its already-locked `tray-icon` feature. Six IPC commands are added: menu presentation, repository reveal, current HEAD ID, status snapshot, restricted status actions and status sizing. The status window capability grants only core event listen/unlisten permissions; its three commands also verify the calling window label. Its action allowlist contains navigation, refresh and existing app controls, with no direct Git mutation commands. No new package dependency, automatic update check or provider integration is introduced.
