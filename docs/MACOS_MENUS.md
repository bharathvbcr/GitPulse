...
Click the icon to open a compact popover. Its header shows the repository switcher, an open-tab count and the branch. Four cards show **Changed**, **Staged**, **Conflicts** and **Stashes**; nonzero counts open the relevant view. A short upstream row shows ahead/behind counts from the **last fetch**. Insight chips appear for a parked merge/rebase, a Git action running in this repository, and work running in other open repositories. One primary action changes with context: **Review changes**, **Resolve conflicts**, **View history** or **Try again**. Quick actions open History, Pulse, Fleet and Terminal, or copy the branch, reveal the folder, open the remote website and toggle appearance. Empty and unavailable states avoid misleading zero counts. Names truncate in the compact view and remain available in tooltips/details. Escape collapses the switcher or details first, then dismisses the panel; `R` refreshes and `1`–`4` open a nonzero metric.

**Details** expands within the panel for path, branch, changed/staged/conflict counts, listed stashes, upstream, watcher state, parked operations and running Git actions. A bounded scroll area keeps the footer reachable. **Refresh**, appearance, copy and repository switching stay in the popover. Reveal and Remote dismiss the panel without bringing GitPulse forward. **Open GitPulse**, the command palette, **Settings** and **Quit** sit in the footer; right-clicking the icon keeps the native Open/Settings/Quit escape path and adds the current primary action and Refresh when they are available. Light/dark appearance, reduced transparency and reduced motion follow the main app's preferences and system settings. The popover still cannot run Git mutations; those stay on the application menu and in the main window.
...
The interactive fixture at `harness/status.html` uses sample data and exercises the production popover component in light/dark, changed, conflict, clean, loading, unavailable, empty and long-name states. Browser interaction checks cover details, repository switching and conflict navigation. These fixture checks validate the rendered component, not the native tray anchor or IPC transport.
