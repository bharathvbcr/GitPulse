...
Follow-up, 2026-09-09: the concurrent-worktree audit introduces schema 20, validates store ownership in the reader, and hardens the canonical queue/writer/watcher paths. See [the worktree audit](DEVMAP_WORKTREE_AUDIT.md). The measurements below describe the earlier staleness change.
...
Across the two canonical repositories, this audit's runtime changes and regressions add 560 lines and remove 70, excluding documentation, generated vendor output, and unrelated concurrent changes. The common freshness policy and affected-test refusal path replace their previous copies rather than adding independent owners.
...
The source changes are implemented and locally tested. No installed GitPulse app or MCP executable was replaced or restarted during this audit; the new client fields and scheduler behavior require a rebuilt consumer. Final repository index rebuild/status probes are recorded under `/tmp/gitpulse-staleness-index-final*` and `/tmp/devmap-staleness-index-final*`.
