...
| **Code graph / repo map** | DevCouncil — rust/ `devmap` | Manvi (already execs `devmap`), GitPulse (new) | Three implementations: Python graph, rust/, Manvi bridge | GitPulse **links the crates in-process** (§8); Python graph frozen to maintenance once devmap reaches parity on top languages. **No indexing code in GitPulse, ever** — there will be no fourth implementation |
