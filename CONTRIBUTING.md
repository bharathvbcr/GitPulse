...
| `windows-test-manifest-contract` | Windows `cargo test --lib` dying at load with `STATUS_ENTRYPOINT_NOT_FOUND` before any unit test runs. The comctl32 v6 manifest was passed only as `rustc-link-arg-tests`, so integration binaries started and the `cdylib` the lib harness loads did not. |
