...
| `npm run check:ipc` | Verifies the Rust `cmd_*` registry (207 handlers) and frontend `invoke()` calls match with zero untracked orphans, and that every `#[tauri::command]` in the crate is actually registered |
...
| `npm run check:types` | Verifies that Rust serde structs match their TypeScript interfaces field-for-field and wire-type-for-wire-type, across 55 contracts (1006 fields) |
...
| `agent-guidance` | Both guides give DevMap precedence, make generated GitNexus rules conditional, and link five installed DevMap skills; Git ignore rules preserve those skills and the plugin marketplace while excluding local agent state. |
...
└── src-tauri/src/        Rust core
    ├── commands/         #[tauri::command] handlers — the ONLY IPC entry points (207 handlers)
