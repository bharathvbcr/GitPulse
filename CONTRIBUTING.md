...
native `canonical_task_briefs_use_native_storage_without_starting_a_model_worker`
test in `src-tauri/src/workbench.rs`. These verify version binding, complete
repository vectors, bounded text, restart persistence and stale-source refusal.
Canonical formatting and corrupt/large storage cases are covered upstream by
Manvi's `crates/dc-store/tests/workbench_briefs.rs`; its Go store tests exercise
the real persistent child. The disposable [workbench harness](harness/README.md)
checks the rendered copy action without changing the OS clipboard.

Terminal preparation and claim tests live in
`src-tauri/src/workbench/terminal_launch.rs`. They create disposable Git
repositories/worktrees and exercise the native adapter against the real profile
store. Upstream `workbench_runs.rs` covers one-use claims, concurrent claim races,
...
| `npm run check:ipc` | Verifies the Rust `cmd_*` registry (225 handlers) and frontend `invoke()` calls match with zero untracked orphans, and that every `#[tauri::command]` in the crate is actually registered |
...
| `npm run check:types` | Verifies that Rust serde structs match their TypeScript interfaces field-for-field and wire-type-for-wire-type, across 67 contracts (169 structs, 1188 fields) |
...
| `menu-state-contract` | A native-menu payload the Rust validator refuses. `cmd_set_menu_state` validates before it applies and a refusal applies nothing, so one malformed payload does not degrade the menu bar, the tray and the status popover — it freezes all three on the last state Rust accepted, while the frontend logs `desktop:menu-state` and retries. Nothing on the TypeScript side enforced the rules, so the suite could not tell a payload that renders from one that can never be sent. This reads every ceiling, enum, action id and refusal message out of `state.rs` and `actions.rs` and holds `menuContract.ts` to them; `menuContract.test.ts` then uses that mirror to prove `buildMenuState` is total. |
...
| `windows-test-manifest-contract` | Windows `cargo test --lib` dying at load with `STATUS_ENTRYPOINT_NOT_FOUND` before any unit test runs. `rustc-link-arg-tests` covers `tests/*.rs` and tauri-winres covers named bins; the lib harness is neither. Catch-all `/MANIFEST:EMBED` reaches it, but the same flag while tauri-winres still embeds RT_MANIFEST is CVT1100 on `gitpulse.exe`. |
...
| `platform-vocabulary-contract` | A component writing one platform's words, or one platform's modifier glyphs, for every host. Four settings surfaces and twenty tooltips shipped macOS wording and raw ⌘/⌥/⌃ to Windows and Linux readers, and none of it is visible to anyone developing on a Mac — which is everyone who wrote it. The command palette's hand-rolled `replaceAll("⌘"…)` is the same shape: it covered two of the four glyphs, so the terminal dock's `⌃\`` reached Windows intact. Platform words and glyph rewriting therefore have one owner, `src/lib/ui/platformCopy.ts`. |
...
└── src-tauri/src/        Rust core
    ├── commands/         #[tauri::command] handlers — the ONLY IPC entry points (225 handlers)
