<script lang="ts">
  ...
  import { explainError, getTask, getTaskBrief, newID, putTask, request, STATUSES, STATUS_LABELS, taskDraft, taskWrite, WorkbenchError, type EnhancementField, type Repository, type Task, type TaskDraft, type WorkspaceCard } from "../workbench/client";
  let { value, repositories, workspaces, primary = "", home = null, active = true, onSaved, onClose }: {
    value: Task | null; repositories: Repository[]; workspaces: WorkspaceCard[];
...
<aside class="task-editor" aria-label={current ? "Task details" : "New task"}>
  ...
  <form onsubmit={(e) => { e.preventDefault(); void save(); }} oninput={() => { dirty = true; }}>
    <fieldset disabled={saving || pending !== null || enhancementBusy}>
      ...
      <fieldset class="repositories"><legend>Linked repositories</legend>{#each repositories as repo (repo.id)}<label class="check"><input type="checkbox" checked={draft.repository_ids.includes(repo.id)} onchange={(e) => membership(repo.id, e.currentTarget.checked)} />{repo.name}</label>{/each}
      {#each draft.repository_ids.filter((id) => !repositories.some((r) => r.id === id)) as missing (missing)}<small>Linked repository {missing} (load more repositories to edit)</small>{/each}</fieldset>
      <label>Primary repository<select bind:value={draft.primary_repository_id} required><option value="" disabled>Select a linked repository</option>{#each draft.repository_ids as repo (repo)}<option value={repo}>{repositories.find((r) => r.id === repo)?.name ?? repo}</option>{/each}</select></label>
    </fieldset>
    {#if error}<p role="alert" class="error">{error}</p>{/if}
    {#if pending}<p>The save result is uncertain. Retry the same write to reconcile it before editing further.</p>{/if}
    <footer><button class="primary" disabled={saving || enhancementBusy || !draft.repository_ids.length} type="submit">{saving ? "Saving…" : pending ? "Retry save" : "Save task"}</button>{#if current}<button type="button" onclick={reload} disabled={saving || enhancementBusy}>Reload saved</button><button type="button" onclick={copy} disabled={copying || saving || enhancementBusy || pending !== null}>{copying ? "Loading brief…" : "Copy saved brief"}</button><button type="button" onclick={remove} disabled={saving || enhancementBusy || pending !== null}>Delete</button>{/if}</footer>
