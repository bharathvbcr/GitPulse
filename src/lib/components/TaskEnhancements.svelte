<script lang="ts">
  ...
  async function act(method: EnhancementMutation) {
    ...
    if (method === "enhancements.recover") {
      if (!await askConfirm({title:"Resolve uncertain attempt?",message:"The provider outcome is unknown. Starting again may create another model call.",confirmLabel:"Resolve attempt"}) || disposed) return;
...
<section class="enhancements" class:quick aria-label="Manvi task enhancements">
  ...
  {#if opened}
    ...
    <fieldset disabled={disabled || controlsLocked}>
      <details open={!quick || !provider || !model}><summary>Model settings{#if provider && model}<span>{provider} · {model}</span>{/if}</summary><div class="pair"><label>Provider<input bind:value={provider} maxlength="128" list="enhancement-providers" oninput={() => { configurationEdited = true; }} /></label><label>Model<input bind:value={model} maxlength="512" placeholder="Choose a model" oninput={() => { configurationEdited = true; }} /></label></div>
      </details>
      ...
      {/each}</div>
      <button class="primary" type="button" onclick={quick ? startEnhancement : generate} disabled={liveAttempt || !provider.trim() || !model.trim() || !availableFields.length}>{preparing ? "Preparing Manvi…" : quick ? "Enhance with Manvi" : "Generate suggestion"}</button>
    </fieldset>
...
<style>
  .advanced-hidden{display:none}h4{font-size:12px;margin:10px 0 6px}.quick .primary{min-height:34px}.quick>fieldset>.primary{width:100%;margin-top:8px}
  summary{cursor:pointer;padding:8px 0;font-size:11px;color:rgb(var(--c-text-muted))}summary span{display:block;overflow-wrap:anywhere;margin-top:4px}.quick{margin-top:8px;padding-top:8px}
