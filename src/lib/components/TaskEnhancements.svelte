...
<section class="enhancements" aria-label="Manvi task enhancements">
  ...
  {#if opened}
    ...
    <fieldset disabled={disabled || controlsLocked}>
      <div class="pair"><label>Provider<input class="gp-field" bind:value={provider} maxlength="128" list="enhancement-providers" oninput={() => { configurationEdited = true; }} /></label><label>Model<input class="gp-field" bind:value={model} maxlength="512" placeholder="Choose a model" oninput={() => { configurationEdited = true; }} /></label></div>
      ...
      {/each}
      <button class="gp-btn-primary" type="button" onclick={generate} disabled={!provider.trim() || !model.trim() || !availableFields.length}>Generate suggestion</button>
    </fieldset>
    {#if configurationError}<p role="alert" class="error">Manvi configuration: {configurationError}</p>{/if}
    {#if error}<p role="alert" class="error">{error}</p>{/if}
    {#if pending}<p>The result needs reconciliation. Task editing is paused to preserve your saved revision.</p><button class="gp-btn" type="button" disabled={busy} onclick={() => { if (pending) void mutate(pending.method, pending.input); }}>Retry pending action</button>{/if}
    <div class="history-heading"><strong>Suggestions</strong><button class="gp-btn" type="button" disabled={loading || controlsLocked} onclick={() => history()}>Refresh</button></div>
    ...
    {#if proposal}
      <article aria-label="Enhancement review">
        ...
        {/if}
        <div class="actions">
          {#if editing}<button class="gp-btn-primary" type="button" disabled={acting || !revisionDirty || (proposal.fields.includes("title") && !revisionDraft.title?.trim())} onclick={saveSuggestion}>Save suggestion edits</button><button class="gp-btn" type="button" disabled={acting} onclick={() => { editing = false; revisionDraft = {}; }}>Discard suggestion edits</button>{/if}
          {#if proposal.state === "ready"}<button class="gp-btn-primary" type="button" disabled={disabled || controlsLocked || !selected.length} onclick={() => act("enhancements.accept")}>Accept selected fields</button><button class="gp-btn" type="button" disabled={disabled || controlsLocked} onclick={editSuggestion}>Edit suggestion</button>{/if}
          {#if proposal.state === "accepted"}<button class="gp-btn" type="button" disabled={disabled || acting} onclick={() => act("enhancements.undo")}>Undo accepted fields</button>{/if}
          {#if proposal.state === "pending" && now < proposal.expires_at}<button class="gp-btn" type="button" disabled={disabled || acting} onclick={() => act("enhancements.generate")}>Start generation</button>{/if}
          {#if ["pending", "ready", "failed", "running"].includes(proposal.state)}<button class="gp-btn" type="button" disabled={disabled || controlsLocked} onclick={() => act("enhancements.dismiss")}>{proposal.state === "running" ? "Request cancellation" : "Dismiss"}</button>{/if}
          {#if ["running", "cancel_requested"].includes(proposal.state) && now >= proposal.expires_at}<button class="gp-btn" type="button" disabled={disabled || acting} onclick={() => act("enhancements.recover")}>Resolve uncertain attempt</button>{/if}
        </div>
...
<style>
  textarea{width:100%;padding:8px;border:1px solid rgb(var(--c-border));border-radius:6px;background:var(--mac-fill-bg,rgb(var(--c-bg)));color:inherit;resize:vertical}
  .enhancements{border-top:1px solid rgb(var(--c-border));margin-top:18px;padding-top:12px;font-size:12px}.heading{display:flex;width:100%;justify-content:space-between;text-align:left;font-weight:650;padding:8px 0}.pair{display:flex;gap:10px}label{display:flex;flex:1;flex-direction:column;gap:5px;margin:10px 0;min-width:0}fieldset{border:0;margin:0;padding:0}.check{flex-direction:row;align-items:center;gap:7px}.check input{width:auto}input{width:100%;min-width:0;padding:7px;border:1px solid rgb(var(--c-border));border-radius:6px;background:var(--mac-fill-bg,rgb(var(--c-bg)));color:inherit}button{padding:6px 9px;border:1px solid rgb(var(--c-border));border-radius:6px}button:disabled{opacity:.5}button:hover:enabled{background:var(--mac-fill-surface-hover,rgb(var(--c-surface-hover)))}.actions,.history-heading{display:flex;align-items:center;gap:7px;flex-wrap:wrap}.history-heading{justify-content:space-between;margin:16px 0 8px}.history{max-height:150px;overflow:auto;display:flex;flex-direction:column;gap:5px}.history button{text-align:left}.history small{display:block}.history .selected{border-color:rgb(var(--c-accent))}small,.hint{color:rgb(var(--c-text-muted));font-size:11px}p{margin:9px 0;line-height:1.5}.error{color:#dc6565}h3{font-size:13px;margin:14px 0 4px}.field-review{margin:12px 0}pre{white-space:pre-wrap;overflow-wrap:anywhere;max-height:170px;overflow:auto;font:12px/1.5 inherit;background:var(--mac-fill-bg,rgb(var(--c-bg)));padding:9px;border:1px solid rgb(var(--c-border));border-radius:6px;margin:4px 0 8px}.suggestion{border-left:3px solid rgb(var(--c-accent))}
</style>
