...
<section class="task-runs" aria-label="Task agent runs">
  ...
  <fieldset disabled={disabled || busy || pending !== null}>
    <label>Repository<select class="gp-select" bind:value={selectedRepository}><option value="">Primary repository</option>{#each task.repository_ids as id (id)}<option value={id}>{repositories.find((repo) => repo.id === id)?.name ?? id}</option>{/each}</select></label>
    <label>Checkout path<input class="gp-field" bind:value={checkout} placeholder="Choose this repository’s working checkout" /><button class="gp-btn" type="button" onclick={browse}>Browse…</button></label>
    <label>Coding agent<select class="gp-select" bind:value={provider} onchange={() => { if (provider === "claude") kind = "external_terminal"; }}><option value="codex">Codex</option><option value="claude">Claude Code</option></select></label>
    <label>Connection<select class="gp-select" bind:value={kind}><option value="external_terminal">Provider terminal</option><option value="managed" disabled={provider !== "codex"}>Managed Codex</option></select></label>
    {#if provider === "claude"}<p>Claude Code currently supports terminal handoffs.</p>{/if}
    <label>Permission mode<select class="gp-select" bind:value={permission} onchange={() => { acknowledged = false; }}>{#each PERMISSION_MODES as mode}<option value={mode}>{labels[mode]}</option>{/each}</select></label>
  ...
  <button class="gp-btn" type="button" onclick={launch} disabled={disabled || busy || (!pending && (!checkout || (permission === "bypass" && !acknowledged)))}>{busy ? "Preparing…" : pending ? "Retry preparation" : kind === "managed" ? "Start managed Codex" : `Launch in ${provider === "claude" ? "Claude Code" : "Codex"}`}</button>
  ...
  <div class="history-heading"><h4>Run history</h4><button class="gp-btn" type="button" onclick={() => refresh()} disabled={loading}>Refresh</button></div>
  ...
  {#each runs as run (run.id)}
    <article>
      ...
      {#if run.kind === "external_terminal" && ["prepared", "starting", "running"].includes(run.state)}<button class="gp-btn" type="button" onclick={() => reopen(run)} disabled={busy}>Open terminal</button>{/if}
      {#if run.kind === "managed"}
