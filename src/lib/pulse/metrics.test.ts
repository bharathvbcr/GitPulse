...
describe("computeHeatmap", () => {
  ...
  it("can assign level 4 when there is a distinct peak", () => {
    const fixedNow = new Date(2026, 8, 2, 12, 0, 0).getTime();
    const dayMs = 24 * 60 * 60 * 1000;
    const commits = [1, 2, 3, 8].flatMap((count, i) => {
      const ts = Math.floor((fixedNow - i * dayMs) / 1000);
      return Array.from({ length: count }, () => createMockCommit({ timestamp: ts }));
    });
    const weeks = computeHeatmap(commits, 2, fixedNow, "count");
    const levels = weeks.flatMap((w) => w.days.map((d) => d!.level));
...
describe("computeHygiene", () => {
  it("calculates percentages and median churn", () => {
    ...
    expect(hygiene.coAuthorCount).toBe(0);
    expect(hygiene.medianChurn).toBe(10);
  });

  it("counts co-authors from the trailer field, never the subject", () => {
    const commits = [
      createMockCommit({
        summary: "feat: add feature",
    ...
    expect(hygiene.coAuthorPercentage).toBe(50);
  });
});
...
describe("computeHotspotRisks", () => {
  it("joins churn with coverage report to calculate risk items", () => {
    ...
    expect(risks[0].coverageStatus).toBe("hit");
  ...
  it("does not treat an unscanned coverage report as untested", () => {
    const topFiles = [
      { path: "src/risky.ts", additions: 500, deletions: 200, commits_count: 15 },
    ];
    const risks = computeHotspotRisks(topFiles, null);
    expect(risks[0].coverageStatus).toBe("unscanned");
    expect(risks[0].coveragePercentage).toBeNull();
    expect(risks[0].riskLevel).not.toBe("critical");
  });
});
