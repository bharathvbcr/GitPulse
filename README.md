...
flowchart TB
    ...
    subgraph IPC["Tauri 2 IPC Boundary (Type-Safe & Contract-Checked)"]
        direction TB
        IPCBridge["<code>invoke('cmd_*', payload)</code><br/><i>(123 Handlers verified by <code>npm run check:ipc</code>)</i>"]
    end
...
| `npm run check:ipc` | Verify 123 Rust commands match frontend `invoke()` calls with zero drift |
