...
flowchart TB
    ...
    subgraph IPC["Tauri 2 IPC Boundary (Type-Safe & Contract-Checked)"]
        direction TB
        IPCBridge["<code>invoke('cmd_*', payload)</code><br/><i>(219 Handlers verified by <code>npm run check:ipc</code>)</i>"]
    end
...
| `npm run check:ipc` | Verify 219 Rust commands match frontend `invoke()` calls with zero drift |
| `npm run check:vendor-schema` | Pin vendored DevMap store schema against the installed `devmap` CLI |
| `npm run check:types` | Compare Rust serde structs and TypeScript interfaces across 64 contracts (153 structs, 1088 fields) |
